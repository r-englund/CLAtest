...
void TransferFunctionEditor::repositionPoints(){
    ...
    for (size_t i = 0; i < points_.size(); ++i){
        float zoomRangeMin = getZoomRangeMin();
        float zoomRangeMax = getZoomRangeMax();
        float datapointXpos = points_[i]->getPoint()->getPos()->x;
        float datapointYpos = points_[i]->getPoint()->getPos()->y;

        float newX = (datapointXpos - zoomRangeMin)  * viewWidth_ / (zoomRangeMax - zoomRangeMin);
        float newY = datapointYpos * viewHeight_;
        points_[i]->setPos(newX, newY);
    }
    this->update();
    this->invalidate();
}

const float TransferFunctionEditor::getZoomRangeMin(){return zoomRangeMin_;}

void TransferFunctionEditor::setZoomRangeMin(float min){zoomRangeMin_ = min;}

const float TransferFunctionEditor::getZoomRangeMax(){return zoomRangeMax_;}

void TransferFunctionEditor::setZoomRangeMax(float max){zoomRangeMax_ = max;}
