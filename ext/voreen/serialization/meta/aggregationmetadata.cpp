...
void AggregationMetaData::setProcessors(const std::vector<inviwo::Processor*>& processors) {
    processors_ = processors;
}

std::vector<inviwo::Processor*> AggregationMetaData::getProcessors(bool recursive) const {
    if (recursive) {
        std::vector<inviwo::Processor*> result = processors_;

        for (size_t i = 0; i < aggregations_.size(); ++i) {
            std::vector<inviwo::Processor*> r = aggregations_[i]->getProcessors(true);
...
void AggregationMetaData::addPortConnection(inviwo::Port* outport, inviwo::Port* inport) {
    inviwo::PortConnection connection(outport, inport);
    portConnections_.push_back(connection);
}

const std::vector<inviwo::PortConnection>& AggregationMetaData::getPortConnections() const {
